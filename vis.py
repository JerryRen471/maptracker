...
work_dir = Path("/data/maptr_workspace/work_dirs/debug_overfit6_stage1_front_roi")
cfg_path = Path("/root/maptracker/subset/debug_overfit6_stage1_front_roi.py")
sub_path = work_dir / "submission_vector.json"
out_root = work_dir / "clean_overfit6_vis"
...
def save_scene(thr, scene):
    out_dir = out_root / f"score_thr_{thr:.2f}" / scene
    ...
    for local_idx, ds_idx in enumerate(scene_indices[scene]):
        ...
        divider_counts.append(len(pred_by_label.get(1, [])))
        ...
        draw_vectors(pred_by_label, pred_path, f"Pred score>={thr:.2f} {local_idx:04d}")
    ...
    if frames:
        ...
        gif_frames = [Image.fromarray(f).resize((f.shape[1] // 2, f.shape[0] // 2)) for f in frames]
        imageio.mimsave(out_dir / "gt_pred.gif", gif_frames, duration=100)
    ...
    return {
        ...
        "maybe_normalized": maybe_normalized,
        ...
        "divider_min": int(min(divider_counts)) if divider_counts else 0,
        "divider_med": float(np.median(divider_counts)) if divider_counts else 0.0,
        "divider_max": int(max(divider_counts)) if divider_counts else 0,
    }

for thr in [0.05, 0.10, 0.20, 0.30]:
    print("\nthreshold", thr)
    for scene in sorted(scene_indices.keys()):
        stat = save_scene(thr, scene)
        print(scene, stat)
